...
BUILD_DIR     = /home/treed/code/parrot
LOAD_EXT      = .so
O             = .o
EXE           = 
MAKE          = make -C
...
# PMCS        = mekso
# PMC_SOURCES = $(PMC_DIR)/mekso.pmc
# MEKSO_GROUP  = $(PMC_DIR)/mekso_group$(LOAD_EXT)
...
mekso$(EXE): mekso.pbc
	$(PBC_TO_EXE) mekso.pbc
	

# the compiler .pbc
mekso.pbc: Makefile $(PARROT) $(SOURCES) $(BUILTINS_PIR)
	$(PARROT) $(PARROT_ARGS) -o mekso.pbc mekso.pir
...
$(MEKSO_GROUP): Makefile $(PARROT) $(PMC_SOURCES)
	cd $(PMC_DIR) && $(BUILD_DYNPMC) generate $(PMCS)
	cd $(PMC_DIR) && $(BUILD_DYNPMC) compile $(PMCS)
	cd $(PMC_DIR) && $(BUILD_DYNPMC) linklibs $(PMCS)
	cd $(PMC_DIR) && $(BUILD_DYNPMC) copy --destination=$(PARROT_DYNEXT) $(PMCS)

src/ops/mekso_ops$(LOAD_EXT) : $(PARROT) $(OPS_FILE)
...
release: manifest
	...
	[ -d mekso-$(VERSION) ] || ln -s . mekso-$(VERSION)
	$(PERL) -ne 'print "mekso-$(VERSION)/$$_"' MANIFEST | \
	    tar -zcv -T - -f mekso-$(VERSION).tar.gz
	rm mekso-$(VERSION)
